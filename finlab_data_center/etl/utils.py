...
def year_transfer(t, method='string'):
    """""
    民國年日期轉換
    """""
    try:
...
class SqlCreateTable:
    ...
    def generate_table(self, db, table, tail):
        series = self.df.iloc[0]
        title = f'CREATE TABLE `{db}`.`{table}` (\n  `id` INT NOT NULL AUTO_INCREMENT,'
        print(title)
        for i in range(len(series)):
            col = list(series.index)
            v = series.iloc[i]
            col_ = col[i]
            print(self.generate_col(col_, v))
        tail = tail
        print(tail)
